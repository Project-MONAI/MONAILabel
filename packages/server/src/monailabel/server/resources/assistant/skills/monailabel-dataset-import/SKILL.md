...
Combined percentage requests use import_dataset_split. Its include_masks argument controls only the annotation/training portion; evaluation cases always receive their supplied labels. For “80% images only for annotation and 20% images+labels for evaluation”, send evaluation_percentage=20 and include_masks=false. A request for evaluation labels is never a reason to set include_masks=true; use true only when the user also requests reference masks for the annotation/training portion. Omit limit unless a total count was requested: percentages alone use the entire labeled source section. One job imports BOTH portions. This is independent of model validation percentages. Imports reuse cached archives, protect existing annotations and leave imported labels pending review.
