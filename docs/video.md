...
CVAT chat, tracking and submission work through localhost or a network HTTP/HTTPS address. Draft checks bind proposals to the exact native draft and viewer session; reloading the native editor requires a fresh proposal.

## Annotate, track and review

Videos, images and volumes share the Datasets table, search, filters, pagination and file selection. The Type column identifies each sample and its row offers the supported viewer: CVAT, QuPath, Slicer or OHIF. Use the sample actions menu for metadata and deletion. Accepted polygon ranges are eligible for segmentation training; selecting an unreviewed clip does not make its frames training labels.

1. Import the sample above, or use **Datasets → Import video** for MP4/MOV, Matroska/WebM or AVI files up to 2 GiB. Enter instrument labels and a patient/procedure ID shared by related clips and exported images. **Evaluation only** reserves the procedure from all training; temporal tracking metrics remain unimplemented.
...
5. In **Reviews**, inspect each submitted frame range, step through its source frames, and choose Good or Needs changes. To correct it, reopen CVAT, edit and submit the changed range before reviewing its new revision. Unaffected ranges keep their decisions. Review decisions apply to submitted tracks, never unsaved drafts.

The embedded assistant shares the workspace conversation model and typed tools. With the video open, ask:

```text
Use GPT-6 Astra to put a bounding box around the snare on this frame.
```
...
```text
Locate the snare and track it for 16 frames.
```
...
With no model named or selected, the backend uses a compatible project/target default, a dedicated target model, or GPT-6 Astra. No manual model selection or starting box is needed. An explicitly named annotation model overrides the panel selection for that request. The conversation model does not implicitly become the annotation model. Compatible configured 2D models can locate or segment the starting frame: GPT vision providers use their registered endpoint, model ID and credential reference; mask providers and local segmentation models use their existing adapters and supported labels. A model must support the selected label and 2D source frame. SAM 2.1 is currently the only temporal tracker. It propagates the chosen model's initial annotation; GPT does not independently process every subsequent frame. Selecting or opening a model does not invoke it. Proposals record the annotation model and, when used, tracker provenance. Inspect and correct the result before submitting.
...
If a visible, unlocked rectangle or polygon track is selected, an ordinary tracking request follows it using SAM. With no track selected, the selected or default annotation model finds the tool before tracking. Name the tool when the project has more than one possible label. Asking from workspace chat opens CVAT when the project has one clip; with multiple clips, choose the intended clip's CVAT action. Repeat the request inside CVAT after choosing a starting frame. Tracking from a manual annotation needs no remote vision model; the assistant still requires its conversation model. Automatic rediscovery after tracking loss and tracking metrics remain [planned work](roadmap.md).
...
After inspecting and accepting polygon ranges, create a U-Net for the tool and train it with approved samples. One video can train without evaluation. An 80:20 split requires multiple independent videos/procedure groups; frames from one procedure stay together. Name the trained model in CVAT, for example “Segment the snare on this frame using Snare U-Net”. SAM 2.1 still handles temporal propagation when tracking is requested. Boxes alone do not train segmentation, and this does not train a temporal tracker. See [learning and review rules](workflows.md#review-regions-and-frame-ranges).

## API and data retention

The authenticated API exposes video import/listing, original source download, frame metadata, track revisions, editor jobs, annotation/tracking proposals, original proposal masks, submission and review. See the running server's `/docs` for schemas. Accepted polygon ranges enter segmentation snapshots as original frames with frozen track revisions. Metadata stores presentation-order `timestamps` relative to the first frame and the original `start_time`; original timestamps are `start_time + timestamps[i]`. Boxes use `[left, top, right, bottom]`; polygon points are flat `[x1, y1, x2, y2, ...]` source image edge coordinates. Grouping must be supplied correctly; differently encoded duplicates cannot be inferred automatically.
