...
The coordinator discovers nine packaged [skills](../packages/server/src/monailabel/server/resources/assistant/skills): workspace, dataset import, batch annotation, review, training, evaluation, radiology, pathology and model onboarding. All skill names and folders use the `monailabel-` prefix. The root `skills/` link opens the same source files. Each follows the [Agent Skills specification](https://agentskills.io/specification). The model sees names and descriptions first, calls `load_skill`, then receives that workflow's instructions and tool schemas. Skill selection resets for each prompt; viewer skills require a selected sample. There is no keyword router.
