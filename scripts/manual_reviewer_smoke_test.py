...
def ensure_port_unused(port: int) -> None:
    """Fail fast if another service is already bound to the test port."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        try:
            ...
            raise RuntimeError(
                f"Port {port} is already in use; stop the existing service before running this test"
            ) from exc
...
def wait_for_server_ready(proc):
    ...
    for _ in range(60):
        ...
        if exit_code is not None:
            print(f"❌ Server exited before becoming ready (exit code {exit_code})")
            return False
...
def test_list_images():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("✅ Successfully listed images")
