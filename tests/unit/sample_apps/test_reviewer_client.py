...
class TestReviewerClientTimeouts(unittest.TestCase):
    ...
    def test_generate_report_returns_text_for_csv(self):
        with patch.object(reviewer_client.requests, "get", return_value=MockResponse("col1,col2\n1,2\n")):
            report = self.client.generate_report("csv")

        self.assertEqual(report, "col1,col2\n1,2\n")

    def test_generate_report_returns_text_for_html(self):
        with patch.object(reviewer_client.requests, "get", return_value=MockResponse("<html></html>")):
            report = self.client.generate_report("html")

        self.assertEqual(report, "<html></html>")

    def test_get_versions_handles_request_failures(self):
        with patch.object(
            reviewer_client.requests,
            "get",
            side_effect=requests.exceptions.RequestException("boom"),
        ):
            response = self.client.get_versions("case-001")

        self.assertEqual(response, {"status": "error", "error": "boom"})
