...
class TestInteractions(unittest.TestCase):
    def run_interaction(self, train, compose):
        ...
        # Deepedit should add 2 more tensors for positive and negative clicks
        self.assertEqual(engine.state.batch[0].get("image").shape[0], 3)
        # Label should keep the same num of tensors
        self.assertEqual(engine.state.batch[0].get("label").shape[0], 1)
