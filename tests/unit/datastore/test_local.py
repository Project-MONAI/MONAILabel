...
class TestLocalDatastore(unittest.TestCase):
    ...
    def test_update_label_info_preserves_explicit_empty_last_reviewed(self):
        with tempfile.TemporaryDirectory() as studies:
            image_file = os.path.join(studies, "case-001.nrrd")
            source_label_file = os.path.join(studies, "case-001.seg.nrrd")
            ...
            datastore.update_label_info(
                "case-001",
                DefaultLabelTag.FINAL,
                {"status": "approved", "last_reviewed": ""},
            )

            self.assertEqual(datastore.get_label_info("case-001", DefaultLabelTag.FINAL).get("last_reviewed"), "")

    def test_update_label_info_falls_back_to_existing_ts_without_incoming_review_metadata(self):
        with tempfile.TemporaryDirectory() as studies:
            image_file = os.path.join(studies, "case-001.nrrd")
            source_label_file = os.path.join(studies, "case-001.seg.nrrd")
            ...
            datastore.update_label_info("case-001", DefaultLabelTag.FINAL, {})
