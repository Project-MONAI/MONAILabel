...
class TestConvertHTJ2K(unittest.TestCase):
    ...
    def test_transcode_multiframe_jpeg_ybr_to_htj2k(self):
        ...
        try:
            ...
            transcode_dicom_to_htj2k(
                file_loader=file_loader, skip_transfer_syntaxes=None  # Override default to test JPEG transcoding
            )
            ...
            print(
                f"✓ PhotometricInterpretation updated: {ds_original.PhotometricInterpretation} -> {ds_transcoded.PhotometricInterpretation}"
            )

            # Verify shapes match
            self.assertEqual(original_pixels.shape, transcoded_pixels.shape, "Pixel array shapes should match")
            ...
            self.assertEqual(
                ds_original.NumberOfFrames, ds_transcoded.NumberOfFrames, "NumberOfFrames should be preserved"
            )
    ...
    def test_transcode_dicom_to_htj2k_batch(self):
        ...
        try:
            ...
            self.assertEqual(
                source_names, transcoded_names, "Filenames should match between source and transcoded directories"
            )
            ...
            for source_file, transcoded_file in zip(source_files, transcoded_files):
                ...
                self.assertIn(
                    transfer_syntax, HTJ2K_TRANSFER_SYNTAXES, f"Transfer syntax should be HTJ2K, got {transfer_syntax}"
                )
                ...
                # Verify metadata is preserved
                self.assertEqual(ds_original.Rows, ds_transcoded.Rows, "Image dimensions (Rows) should be preserved")
                self.assertEqual(
                    ds_original.Columns, ds_transcoded.Columns, "Image dimensions (Columns) should be preserved"
                )
                self.assertEqual(
                    ds_original.BitsAllocated, ds_transcoded.BitsAllocated, "BitsAllocated should be preserved"
                )
                self.assertEqual(ds_original.BitsStored, ds_transcoded.BitsStored, "BitsStored should be preserved")
    ...
    def test_transcode_mixed_directory(self):
        ...
        try:
            ...
            self.assertEqual(len(output_files), len(mixed_files), "Output should have same number of files as input")
            ...
            for input_file in mixed_files:
                ...
                if ts_input not in HTJ2K_TRANSFER_SYNTAXES:
                    ...
                    self.assertIn(
                        ts_output, HTJ2K_TRANSFER_SYNTAXES, f"File {input_file.name} should be HTJ2K after transcoding"
                    )
    ...
    def test_transcode_dicom_to_htj2k_multiframe_metadata(self):
        ...
        try:
            ...
            self.assertEqual(
                len(per_frame_seq), num_frames, f"PerFrameFunctionalGroupsSequence should have {num_frames} items"
            )
            ...
            for frame_idx in range(num_frames):
                ...
                self.assertTrue(
                    hasattr(frame_item, "PlanePositionSequence"), f"Frame {frame_idx} should have PlanePositionSequence"
                )
    ...
    def test_transcode_dicom_to_htj2k_multiframe_nifti_consistency(self):
        ...
        try:
            ...
            # Verify shapes match
            self.assertEqual(
                data_original.shape, data_multiframe.shape, "Original and multi-frame should produce same NIfTI shape"
            )
        ...
        for prog_order, expected_ts in progression_orders:
            with self.subTest(progression_order=prog_order):
                ...
                try:
                    ...
                    file_loader = DicomFileLoader(input_dir, output_dir)
                    transcode_dicom_to_htj2k(file_loader=file_loader, progression_order=prog_order)
                    ...
                    self.assertEqual(
                        transcoded_ts, expected_ts, f"Transfer syntax should be {expected_ts} for {prog_order}"
                    )
            ...
            for invalid_order in invalid_orders:
                with self.subTest(invalid_progression_order=invalid_order):
                    ...
                    with self.assertRaises(ValueError) as context:
                        file_loader = DicomFileLoader(input_dir, output_dir)
                        transcode_dicom_to_htj2k(file_loader=file_loader, progression_order=invalid_order)
        ...
        for prog_order, expected_ts in progression_orders:
            with self.subTest(progression_order=prog_order):
                ...
                try:
                    ...
                    result_dir = convert_single_frame_dicom_series_to_multiframe(
                        input_dir=dicom_dir, output_dir=output_dir, convert_to_htj2k=True, progression_order=prog_order
                    )
    ...
    def test_skip_transfer_syntaxes_htj2k(self):
        ...
        try:
            ...
            self.assertEqual(original_checksum, output_checksum, "File should be identical (copied, not transcoded)")
            self.assertEqual(original_size, output_size, "File size should be identical")

            # Verify transfer syntax is still HTJ2K
            ds_output = pydicom.dcmread(output_file)
            self.assertEqual(
                str(ds_output.file_meta.TransferSyntaxUID), htj2k_ts, "Transfer syntax should be preserved"
            )
    ...
    def test_skip_transfer_syntaxes_mixed_batch(self):
        ...
        try:
            ...
            self.assertEqual(
                ct_original_checksum, ct_output_checksum, "CT file should be identical (copied, not transcoded)"
            )

            ds_ct_output = pydicom.dcmread(ct_output)
            self.assertEqual(
                str(ds_ct_output.file_meta.TransferSyntaxUID), ct_ts, "CT transfer syntax should be preserved (LRCP)"
            )
            ...
            self.assertEqual(mr_ts_output, "1.2.840.10008.1.2.4.202", "MR should be transcoded to RPCL HTJ2K")

            # Verify MR pixels are lossless
            mr_pixels_output = ds_mr_output.pixel_array
            np.testing.assert_array_equal(mr_pixels_orig, mr_pixels_output, err_msg="MR pixels should be lossless")
    ...
    def test_skip_transfer_syntaxes_multiple(self):
        ...
        try:
            ...
            shutil.copy2(os.path.join(htj2k_dir1, file1_name), os.path.join(input_dir, file1_name))
            ...
            shutil.copy2(os.path.join(htj2k_dir2, file2_name), os.path.join(input_dir, file2_name))
    ...
    def test_skip_transfer_syntaxes_override_to_transcode_all(self):
        ...
        try:
            ...
            file_loader = DicomFileLoader(input_dir, output_dir)
            transcode_dicom_to_htj2k(
                file_loader=file_loader, skip_transfer_syntaxes=None  # Override default to force transcoding
            )
    ...
    def test_transcode_with_pytorch_dataloader(self):
        ...
        try:
            ...
            source_dir = os.path.join(
                self.dicom_dataset, "1.2.826.0.1.3680043.8.274.1.1.8323329.686405.1629744173.656721"
            )
            ...
            for filename in test_files:
                ...
                np.testing.assert_array_equal(
                    original_pixels, transcoded_pixels, err_msg=f"Pixels should match for {filename}"
                )
