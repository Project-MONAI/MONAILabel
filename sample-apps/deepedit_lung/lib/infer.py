...
class Deepgrow(InferTask):
    ...
    def pre_transforms(self):from monai.apps.deepgrow.transforms import AddGuidanceFromPointsd, AddGuidanceSignald
from monai.inferers import SimpleInferer
from monai.transforms import (
    Activationsd,
    AddChanneld,
...
class Segmentation(InferTask):
    ...
    def __init__(
        ...
        labels="lung",
        dimension=3,
        description="A pre-trained model for volumetric (3D) segmentation of the lung over 3D CT Images",
    ):
        super().__init__(
            path=path,
            network=network,
