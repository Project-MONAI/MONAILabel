...
class DeepEdit(InferTask):
    ...
    def pre_transforms(self):
        return [
            # TODO:: THESE TRANSFORMS ASSUME UI_preds path exist -- Path to predictions generated by UI
            LoadImaged(keys=("image", "UI_preds"), reader="ITKReader"),
            AddChanneld(keys=("image", "UI_preds")),
            # Spacingd(keys="image", pixdim=self.target_spacing, mode="bilinear"),
            Orientationd(keys=("image", "UI_preds"), axcodes="RAS"),
            # This transform may not work well for MR images
            ScaleIntensityRanged(
                keys="image",
                a_min=-175,
            ...
            Resized(keys=("image", "UI_preds"), spatial_size=self.spatial_size, mode="area"),
            ResizeGuidanceMultipleLabelCustomd(guidance="guidance", ref_image="image"),
            AddGuidanceSignalAndPredsd(keys="image", guidance="guidance", step="deepgrow_infer"),
            ToTensord(keys="image"),
        ]
