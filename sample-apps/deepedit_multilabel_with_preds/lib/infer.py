...
class DeepEdit(InferTask):
    ...
    def pre_transforms(self):
        return [
            # THESE TRANSFORMS ASSUME UI_label path exist -- Path to predictions generated by UI
            LoadImaged(keys=("image", "UI_label"), reader="ITKReader"),
            AddChanneld(keys=("image", "UI_label")),
            # Spacingd(keys="image", pixdim=self.target_spacing, mode="bilinear"),
            Orientationd(keys=("image", "UI_label"), axcodes="RAS"),
            # This transform may not work well for MR images
            ScaleIntensityRanged(
                keys="image",
                a_min=-175,
            ...
            Resized(keys=("image", "UI_label"), spatial_size=self.spatial_size, mode="area"),
