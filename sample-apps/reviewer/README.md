...
curl -s "http://localhost:8000/review/report?fmt=json" > review_report.json

# Generate CSV report (extract the CSV content from the JSON envelope)
curl -s "http://localhost:8000/review/report?fmt=csv" | python -c "import json,sys; sys.stdout.write(json.load(sys.stdin)['content'])" > review_report.csv

# Generate HTML report (extract the HTML content from the JSON envelope)
curl -s "http://localhost:8000/review/report?fmt=html" | python -c "import json,sys; sys.stdout.write(json.load(sys.stdin)['content'])" > review_report.html
```
