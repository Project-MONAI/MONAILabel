...
def _json_dict(response: requests.Response) -> Dict[str, Any]:
    ...
    return data if isinstance(data, dict) else {"error": "Invalid JSON response"}
...
class LightweightReviewClient:
    ...
    def download_image(self, image_id: str) -> Optional[bytes]:
        ...
        try:
            ...
            if response.status_code == 200:
                image_data = cast(bytes, response.content)
    ...
    def download_labelinfo(self, label_id: str) -> Optional[Dict[str, Any]]:
        """
        Download label metadata.
    ...
    def update_labelinfo(
        ...
        """
        try:
            review_info: Dict[str, Any] = {"status": status}
            ...
            if workflow_id:
                review_info["workflow_id"] = workflow_id
    ...
    def save_label(
        ...
        try:
            ...
            with open(label_file, "rb") as f:
                upload_name = label_file.name or image_id
                files = {"label": (upload_name, f)}
    ...
    def generate_report(self, fmt: str = "json", reviewer: Optional[str] = None) -> Union[Dict[str, Any], str]:
        """
        ...
        """
        try:
            response_format = fmt.lower()
            response = requests.get(
                f"{self.server_url}/review/report",
                params={"fmt": response_format, "reviewer": reviewer},
                timeout=self.timeout,
                headers=self.headers,
            )
