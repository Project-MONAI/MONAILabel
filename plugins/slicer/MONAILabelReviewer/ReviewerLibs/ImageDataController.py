...
class ImageDataController:
    ...
    def getAllImageData(self, segmented, isNotSegmented, isApproved, isFlagged) -> List[ImageData]:
        return self.imageDataExtractor.getAllImageData(segmented=segmented, notSegmented=isNotSegmented, approved=isApproved, flagged=isFlagged)
    
    def getImageDataByClientId (self, selectedClientId, isApproved, isFlagged) -> List[ImageData]:
        return self.imageDataExtractor.getImageDataByClientId(clientId=selectedClientId, approved=isApproved, flagged=isFlagged)
    ...
    def searchByAnnotatorReviewer(self, selectedAnnotator : str, selectedReviewer : str, isApproved : bool, isFlagged : bool) -> Dict[str, ImageData]:
        '''
        ...
        if(selectedAnnotator == "All" and selectedReviewer != "All"):
            imageIdsOfAnnotator = self.imageDataExtractor.getImageDataByReviewer(selectedReviewer, isApproved, isFlagged)

        if(selectedAnnotator != "All" and selectedReviewer == "All"):
            imageIdsOfAnnotator = self.imageDataExtractor.getImageDataByClientId(selectedAnnotator, isApproved, isFlagged)

        if(selectedReviewer == "All" and selectedAnnotator == "All"):
            imageIdsOfAnnotator = self.imageDataExtractor.getAllImageData(segmented=True, notSegmented=False, approved=isApproved, flagged=isFlagged)

        if(selectedReviewer != "All" and selectedAnnotator != "All" ):
            imageIdsOfAnnotator = self.imageDataExtractor.getImageDataByClientAndReviewer(selectedAnnotator, selectedReviewer, isApproved, isFlagged)
    ...
    def getImageDataByLevel(self, isEasy : bool, isMedium : bool, isHard : bool) -> Dict[str, ImageData]:
        ...
        '''
        imageIdsOfAnnotator = self.imageDataExtractor.getImageDataByLevel(isEasy=isEasy, isMedium=isMedium, isHard=isHard)
    ...
    def updateLabeInfo(self, imageId, updatedMetaJson) -> bool:
        ...
        if(repsonseCode == 200):
            logging.info("{}: Successfully persist meta data for image (id='{}')".format(self.getCurrentTime(), imageId))
            return True
        else:
            logging.info("{}: Failed meta date persistence for image (id='{}', response code = '{}')".format(self.getCurrentTime(), imageId, repsonseCode))
            return False
    ...
    def reuqestSegmentation(self, image_id) -> requests.models.Response:
        ...
        logging.info("{}: Segmentation successfully requested from MONAIServer (image id: {})".format(self.getCurrentTime(), image_id))
        return img_blob
