...
#Section: Loading images
  ...
  def getPercentageApproved(self, selectedClientId):
      percentageApprovedOfClient, idxApprovedOfClient = self.imageDataController.getPercentageApproved(selectedClientId)  
      return percentageApprovedOfClient, idxApprovedOfClient

  def getPercentageSemgmentedByClient(self, selectedClientId):
      percentageSemgmentedByClient,  idxSegmentedByClient= self.imageDataController.getPercentageSemgmentedByClient(selectedClientId)
...
#Section: Search Image
  ...
  def searchByAnnotatorReviewer(self, selectedAnnotator : str, selectedReviewer : str, isApproved : bool, isFlagged : bool) -> Dict[str, ImageData]:
    return self.imageDataController.searchByAnnotatorReviewer(selectedAnnotator, selectedReviewer, isApproved, isFlagged)
...
#Section: Dicom stream
  ...
  def loadDicomAndSegmentation(self, imageData):
    ...
    logging.info("{}: Request Data  image_name='{}', node_name='{}', image_id='{}', checksum='{}'".format(self.getCurrentTime(), image_name, node_name, image_id, checksum))
    ...
    if(imageData.isSegemented()):
      ...
      logging.info("{}: Removed file at {}".format(self.getCurrentTime(), destination))

  def storeSegmentation(self, response : requests.models.Response, segmentationFileName : str, tempDirectory : str) -> str:
      '''
      ...
      logging.info("{}: Images is stored in: {}".format(self.getCurrentTime(), destination))
      return destination
  ...
  def requestDicomImage(self, image_id : str, image_name : str, node_name : str, checksum : str): 
      ...
      _volumeNode = sampleDataLogic.downloadFromURL(nodeNames=node_name, fileNames=image_name, uris=download_uri, checksums=checksum)[0]
  ...
  def setTempFolderDir(self):
    ...
    logging.info("{}: Temporary Directory: '{}'".format(self.getCurrentTime(), self.temp_dir.name))
