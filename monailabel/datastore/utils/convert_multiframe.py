...
def _validate_series_consistency(datasets: List[pydicom.Dataset]) -> dict:
    ...
    if modality not in SUPPORTED_MODALITIES:
        raise ValueError(
            f"Unsupported modality: {modality}. " f"Supported modalities are: {', '.join(SUPPORTED_MODALITIES)}"
        )
    ...
    for attr in required_attrs:
        if not all(hasattr(ds, attr) and getattr(ds, attr) == getattr(first_ds, attr) for ds in datasets):
    ...
    logger.info(
        f"Series validated: {modality} {metadata['rows']}x{metadata['columns']}, " f"{metadata['num_frames']} frames"
    )
...
def convert_to_enhanced_dicom(
    ...
    if not _check_highdicom_available():
        raise ImportError("highdicom is not installed. Install it with: pip install highdicom")
...
def batch_convert_by_series(
    ...
    **compression_kwargs,
    ...
    for idx, (series_uid, file_paths) in enumerate(series_files.items(), 1):
        ...
        if metadata["modality"] not in SUPPORTED_MODALITIES:
            ...
            if compress_htj2k:
                ...
                success, transcoded_size_mb = _transcode_files_to_htj2k(file_paths, output_dir, compression_kwargs)
        ...
        with tempfile.TemporaryDirectory() as temp_series_dir:
            ...
            try:
                ...
                if success:
                    ...
                    stats["failed"] += 1
                    stats["series_info"].append(
                        {"series_uid": series_uid, "status": "failed", "reason": "Conversion returned False"}
                    )

            except Exception as e:
                logger.error(f"  Failed to convert series: {e}")
                stats["failed"] += 1
                stats["series_info"].append({"series_uid": series_uid, "status": "failed", "reason": str(e)})
    ...
    if stats["total_size_input_mb"] > 0 and stats["total_size_output_mb"] > 0:
        ...
        size_reduction_pct = (
            (stats["total_size_input_mb"] - stats["total_size_output_mb"]) / stats["total_size_input_mb"]
        ) * 100
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert legacy DICOM series to enhanced multi-frame format")
    parser.add_argument("input", type=str, help="Input directory containing legacy DICOM files")
    parser.add_argument(
        "-o", "--output", type=str, help="Output file path for enhanced DICOM (required unless --validate-only)"
    )
    parser.add_argument("--validate-only", action="store_true", help="Only validate series without creating output")
    ...
    parser.add_argument("--htj2k", action="store_true", help="Compress the enhanced DICOM file with HTJ2K")

    parser.add_argument(
        "--num-resolutions", type=int, default=6, help="Number of wavelet decomposition levels (default: 6)"
    )

    parser.add_argument("--code-block-size", type=tuple, default=(64, 64), help="Code block size (default: (64, 64))")

    parser.add_argument("--progression-order", type=str, default="RPCL", help="Progression order (default: RPCL)")

    parser.add_argument("--preserve-series-uid", action="store_true", help="Preserve the original SeriesInstanceUID")
