...
class EpistemicScoringVideo(ScoringMethod):
    """
    ...
    def run_scoring(self, image_id, simulation_size, model_ts, datastore):
        ...
        entropies = []
        cap = cv2.VideoCapture(datastore.get_image_uri(image_id))
        max_num_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

        for frame_id in range(max_num_frames):
            request = {
        ...
        # get video based entropy, strategies
        # 1 Average stratey:
        entropy_video = np.mean(entropies)
