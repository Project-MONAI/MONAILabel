...
class BasicTrainTask(TrainTask):
    ...
    def __init__(
        ...
        self._train_datalist, self._val_datalist = (
            partition_dataset(
                data_list, ratios=[(1 - val_split), val_split], shuffle=True
            )
            if val_split > 0.0
            else (data_list, [])
        )
        ...
        logger.info(
            f"Total Records for Training: {len(self._train_datalist)}/{len(data_list)}"
        )
        logger.info(
            f"Total Records for Validation: {len(self._val_datalist)}/{len(data_list)}"
        )
    ...
    def train_data_loader(self):
        return DataLoader(
            dataset=PersistentDataset(
                self._train_datalist, self.train_pre_transforms()
            ),
    ...
    def train_key_metric(self):
        return {
            "train_dice": MeanDice(output_transform=lambda x: (x["pred"], x["label"]))
        }

    def train_handlers(self):
        lr_scheduler = torch.optim.lr_scheduler.StepLR(
            self.optimizer(), step_size=5000, gamma=0.1
        )
        ...
        if eval:
            handlers.append(
                ValidationHandler(
                    validator=eval, interval=self._val_interval, epoch_level=True
                )
            )
        ...
        if self._load_path and os.path.exists(self._load_path):
            handlers.append(
                CheckpointLoader(
                    ...
                    load_dict={"model": self.network()}
                    if self._load_dict is None
                    else self._load_dict,
                )
            )
    ...
    def val_data_loader(self):
        return (
            DataLoader(
                dataset=PersistentDataset(
                    self._val_datalist, self.val_pre_transforms()
                ),
    ...
    def val_handlers(self):
        return [
            ...
            TensorBoardStatsHandler(
                log_dir=self._output_dir, output_transform=lambda x: None
            ),
            CheckpointSaver(
                save_dir=self._output_dir,
